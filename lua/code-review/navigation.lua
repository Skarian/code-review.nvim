...
function M.work_window(win)
  if not M.normal_window(win) then
    return false
  end
  local buf = vim.api.nvim_win_get_buf(win)
  return loaded_buf(buf) and vim.bo[buf].buftype == "" and not M.aux_window(win)
end

function M.start_file_window(win)
  return M.work_window(win) and normal_named_file_buf(vim.api.nvim_win_get_buf(win))
end
...
function M.has_work_anchor_window()
  return #M.tab_work_windows(0) > 0
end

function M.tab_work_windows(tab)
  local wins = {}
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(tab or 0)) do
    if M.work_window(win) then
      wins[#wins + 1] = win
    end
  end
  return wins
...
function M.any_tab_has_work_window()
  for _, tab in ipairs(vim.api.nvim_list_tabpages()) do
    if #M.tab_work_windows(tab) > 0 then
      return true
    end
  end
  return false
...
function M.snapshot_work_windows()
  local windows = {}
  for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
    if M.work_window(win) then
      windows[win] = true

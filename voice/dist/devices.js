...
function run(command, args, spawnImpl = nodeSpawn) {
...
function avfoundationDevice(index, name) {
...
function avfoundationOpaqueDevice(index) {
    return {
        id: `ffmpeg-avfoundation:audio:${index}`,
        provider: "ffmpeg-avfoundation",
        kind: "audioinput",
...
function dshowDevice(name, alternativeName) {
...
function dshowOpaqueDevice(stableName) {
    return {
        id: `ffmpeg-dshow:audio:${encodeURIComponent(stableName)}`,
        provider: "ffmpeg-dshow",
        kind: "audioinput",
...
function selectionSummary(device, reason) {
    if (!device)
        return null;
    return { id: device.id, reason };
}
export function resolveOpaqueAudioDevice(value, platform = process.platform) {
    if (!value)
        return null;
...
export async function resolveAudioDevice(value, options = {}) {
    const platform = options.platform || process.platform;
    const opaque = resolveOpaqueAudioDevice(value, platform);
    if (opaque)
        return opaque;
    const listing = await listAudioDevices(options);
    if (listing.ok === false)
        return listing;
    ...
    if (recommendedId) {
        const matchedRecommended = devices.find((device) => device.id === recommendedId);
        if (matchedRecommended)
            return matchedRecommended;
    }
    const defaultId = listing.defaultSelection?.id;
    if (defaultId) {
        const matchedDefault = devices.find((device) => device.id === defaultId);
        if (matchedDefault)

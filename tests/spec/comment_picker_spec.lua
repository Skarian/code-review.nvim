describe("comment picker", function()
  ...
  it("opens edit composer from newest-first picker items with previews", function()
    ...
    vim.api.nvim_buf_set_lines(state.get().composer.buf, state.get().composer.body_start, -1, false, { "edited" })
  ...
  it("appends a visual reference to the selected existing comment", function()
    ...
    assert.equals(comment.id, state.get().current_comment_id)
    code_review.quit()
  end)

  it("deletes and jumps to picker-selected comments", function()
    ...
    picker.adapter.pick_comments = function(items, callbacks)
      ...
      assert.equals(second.id, state.get().current_comment_id)

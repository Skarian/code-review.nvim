describe("lifecycle and keymaps", function()
  it("installs child mappings without mapping the parent prefix", function()
    ...
    assert.equals("", vim.fn.maparg("<leader>rr", "n"))
    assert.truthy(vim.fn.maparg("<leader>rr", "x"))
    assert.truthy(vim.fn.maparg("<leader>re", "n"))
  ...
  it("keeps selected comments as selection state when switching reviews", function()
    local code_review = require("code-review")
    local config = require("code-review.config")
    local actions = require("code-review.actions")
    local state = require("code-review.state")
